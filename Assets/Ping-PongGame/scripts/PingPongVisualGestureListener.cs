...
public class PingPongVisualGestureListener : MonoBehaviour, VisualGestureListenerInterface
{
	...
	private RacketController racketScript;

	void Start ()
	{
		racketScript = racket.GetComponent<RacketController>();
	...
	public bool GestureCompleted(long userId, int userIndex, string gesture, float confidence)
	{
		...
		if (gesture == "arm_Left" || gesture == "arm_Right" )
			racketScript.up ();
